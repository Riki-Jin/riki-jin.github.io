...
| DESeq2  | RLE | Models counts with negative binomial generalized <br> linear models (GLMs). Dispersion estimates are <br> shrunken toward fitted values with empirical <br>  Bayes and log fold change of abundances between <br> groups are compared with a Wald test (Love et al. 2014).|
